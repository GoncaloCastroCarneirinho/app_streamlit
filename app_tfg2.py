# -*- coding: utf-8 -*-
"""
Created on Thu Aug 12 13:02:02 2021
...
# df.columns += '_meteo1'
df_meteo = lee_meteo(pd.date_range(start='2020/11/12', end='2020/11/16', freq='10T'),path_estacion="dataLogger/") #Dataframe de Meteo

#df_datalogger.index=df_datalogger.index.round('T')
...
view_mode_col, col1, col2 = st.beta_columns([2,6,6]) #Estructura superior de las vistas de la app en columnas
...
if view_mode == 'Live': #modalidad LIVE
    ...
    variables_seleccion = col2.multiselect("", df.columns.tolist()) #seleccion de variables a representar
    
    sidebar_live = st.sidebar #creacion de ventana despegable a la izquierda
    ...
    col1.markdown("""<p style='display: block; height:100px; line-height:100px; text-align: center; align: center; font-size: 25px; font-family: calibri; font-weight: bold'>SELECT SET OF VARIABLES TO PLOT LIVE</p>""", unsafe_allow_html=True,) #'SELECCIONAR VARIABLES A REPRESENTAR EN MODO LIVE'
    ...
    first_variables_set_selected_resume = col1.multiselect("SELECT FIRST SET OF VARIABLES", df.columns.tolist()) #seleccion de primero set de variables a representar graficamente
    
    second_variables_set_selected_resume = col2.multiselect("SELECT SECOND SET OF VARIABLES", df.columns.tolist()) #seleccion de primero set de variables a representar graficamente
    
    sidebar_resume = st.sidebar #creacion de ventana desplegable a la izquierda

    sidebar_col1, sidebar_col2 = sidebar_resume.beta_columns([1,1]) #estructuración de ventana desplegable por columnas
    ...
    empty_column, first_set_selection_col, second_set_selection_col = st.beta_columns([2,6,6]) #estructuración de otra fila de columnas en la vista RESUME

    with first_set_selection_col: #funcionalidades en la columna 'variables_set_selection_col'
    ...
    with second_set_selection_col:
        ...
        with st.beta_expander("Pick a plot option:"): #creacion de filtro desplegable para elegir diferentes sets de variables a representar graficamente
            ...
            if  options_list == "SECOND SET ALREADY SELECTED":
                variables_set_1=second_variables_set_selected_resume
            elif options_list == "SECOND TEMPERATURE SET":
                variables_set_1=options_list
            elif options_list == "SECOND HUMIDITY SET":
                variables_set_1=options_list
                
            if variables_set_1 == second_variables_set_selected_resume:
            
                df_filt1 = df[[item for item in second_variables_set_selected_resume if 'TEMP' in item]] #creacion de dataframe personalizado para representaciones grafica y modo tabla de set alternativo 'TEMPERATURAS' de variables
                for col in df_filt1:
                ...
                df_filt1 = df[[item for item in second_variables_set_selected_resume if 'RH' in item]] #creacion de dataframe personalizado para representaciones grafica y modo tabla de set alternativo 'HUMEDADES' de variables
                for col in df_filt1:
                    sidebar_col2.markdown("<p style='display: block; text-align: center; font-size: 20px; font-family: calibri'>"+col, unsafe_allow_html=True,) #publicacion en la ventana desplegable de variables representadas
    ...
    if variables_set_1==second_variables_set_selected_resume:
        ...
        Data_chart11 = Data_chart2.line_chart(filtered_df_date[second_variables_set_selected_resume]) #representacion grafica (grafico 2) de dataframe 1 filtrado por fechas
    ...
    variables_set_selected_live = col2.multiselect(" ", df.columns.tolist()) #seleccion de primero set de variables a representar en modo tabla
    
    col1.markdown("""<a style='display: block; height:100px; line-height:100px; text-align: center; font-size: 25px; font-family: calibri; font-weight: bold'>SELECT SET OF VARIABLES TO SHOW ON TABLE</a>""", unsafe_allow_html=True,) #"ELEGIR VARIABLES A REPRESENTAR EN TABLA"
